  ...
  draw_panel = function(self, data, panel_params, coord, r0 = 0.45, linejoin = "mitre") {
    ...
    polys <- lapply(split(data, seq_len(nrow(data))), function(row) {
      ...
      if(row$r == 0) {
        ...
        aes_sector <- new_data_frame(row[aesthetics])[rep(1, 100), ]
        sector <- GeomPolygon$draw_panel(cbind(s, aes_sector), panel_params, coord)
      }
      if(row$r == 1) {
        line <- grid::nullGrob()
      } else {
        aes_line <- new_data_frame(row[aesthetics])[rep(1, 100), ]
...
#' @noRd
point_to_sector <- function(x, y, r, r0 = 0.48, n = 98) {
  if(r == 0) {
    xx <- c(x, x, x)
    yy <- c(y, y + r0, y)
  } else {
...
#' @noRd
point_to_line <- function(x, y, r, r0 = 0.48, n = 100) {
  sign <- sign(r)
